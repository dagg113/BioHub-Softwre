<?php
// Se incluye el modelo de ambulancia y la conexión a la base de datos.
...
// Se crea una instancia del modelo de ambulancia con la conexión de la base de datos
...
<meta charset="UTF-8">
    ...
    display: block; /* Cambia el comportamiento de la tabla */
}
...
/* Otros estilos para el formulario y modal */
...
/* Estilos para botones de gestión */
.btn-gestion {
...
<script>
    ...
    function toggleFields(value, id = '') {
        // Si hay un ID, se agrega como sufijo a los IDs de los elementos
    const prefix = id ? `-${id}` : '';
    ...
    const categoriaSelect = document.getElementById(`categoria_ambu${prefix}`);

    const conductorSelect = document.getElementById(`conductor_id${prefix}`);
    const equipoSelect = document.getElementById(`equipo_trabajo${prefix}`);
    const planSelect = document.getElementById(`plan${prefix}`);
    const categoriaSelectField = document.getElementById(`categoria_ambu${prefix}`);

// Si el valor es "En mantenimiento", oculta algunos campos y quita los requisitos
    ...
    if (value === 'En mantenimiento') {
        ...
        // No ocultar la categoría aquí
        if (categoriaSelect) categoriaSelect.style.display = 'block'; // O mantenerlo visible

        // Remover atributos 'required'
        if (conductorSelect) conductorSelect.required = false;
        if (equipoSelect) equipoSelect.required = false;
        if (planSelect) planSelect.required = false;
        // Mantener la categoría como 'required' si es necesario
    } else {
        ...
        if (categoriaSelect) categoriaSelect.style.display = 'block';

        // Agregar atributos 'required'
        if (conductorSelect) conductorSelect.required = true;
        if (equipoSelect) equipoSelect.required = true;
        if (planSelect) planSelect.required = true;
        if (categoriaSelectField) categoriaSelectField.required = true;
    }
}
    ...
    function toggleConductorForm(value) {
        ...
        const categoriaSelect = document.getElementById('categoria_ambu');

        const conductorSelect = document.getElementById('conductor_id');
        const equipoSelect = document.getElementById('equipo_trabajo');
        const planSelect = document.getElementById('plan');
        const categoriaSelectField = document.getElementById('categoria_ambu');
...
// Si el valor es "En mantenimiento", oculta algunos campos y quita los requisitos
        ...
        if (value === 'En mantenimiento') {
            ...
            // Remover atributos 'required'
            conductorSelect.required = false;
            equipoSelect.required = false;
            planSelect.required = false;
            categoriaSelectField.required = false;
        } else {
            ...
            // Agregar atributos 'required'
            conductorSelect.required = true;
            equipoSelect.required = true;
            planSelect.required = true;
            categoriaSelectField.required = true;
        }
    }
    // Función para mostrar la información del plan seleccionado en el modal
    function showPlanInfo(value) {
        ...
        const closeModal = document.getElementsByClassName('close')[0];
         // Diccionario con información sobre los planes
        const planInfo = {
            'Básico 1': 'Plan Básico 1: Cobertura básica para emergencias estándar.',
            'Básico 2': 'Plan Básico 2: Cobertura básica extendida con algunos extras.',
            'Básico 3': 'Plan Básico 3: Cobertura básica con servicios adicionales.',
        ...
        // Si el valor es válido, se actualiza el contenido del modal y se muestra
...
<!-- Carga la librería de Bootstrap para funcionalidades adicionales -->
